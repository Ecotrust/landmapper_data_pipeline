...
from .gee_utils import (
    ...
    get_dynamic_world_download_url,
    read_gee_url,
)
...
def colorize_dynamic_world(img):
    """Assigns colors to a Dynamic World land cover image.

    Parameters
    ----------
    img : arr, shape (H, W)
      array containing Dynamic World land cover classifications
...
def s2_from_gee(bbox, year, epsg, scale=10, season="leafon"):
    ...
    epsg : int
      EPSG code used to define projection
    """
    url = get_sentinel2_download_url(bbox, year, epsg, scale, season=season)
    ras, profile = read_gee_url(url)
...
def elevation_point_query_tnm(lon, lat, units="Feet"):
    ...
    params = {"x": lon, "y": lat, "units": units, "includeDate": False, "wkid": 4326}
    r = requests.get(BASE_URL, params=params)
    elev = r.json()["value"]
