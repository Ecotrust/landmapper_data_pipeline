...
def colorize_dynamic_world(img):
    """Assigns colors to an Dynamic World land cover image.

    Parameters
    ----------
    img : arr, shape (H, W)
      array containing DW land cover classifications
    ...
    COLOR_MAP = {
        ...
        1: [57, 125, 73],  # trees
        2: [136, 176, 83],  # grass
        3: [122, 135, 198],  # flooded vegetation
        4: [228, 150, 53],  # crops
        5: [223, 195, 90],  # shrub and scrub
        6: [196, 40, 27],  # built
        7: [165, 155, 143],  # bare
        8: [179, 159, 225],  # snow and ice
    }
    land_color = np.zeros((img.shape[0], img.shape[1], 3), dtype=np.uint8)
    for cov in np.unique(img):
        mask = img == cov
